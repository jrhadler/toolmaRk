...
fixed_width_with_modeling <- function(dat1, dat2, coarse = .25, fine = .01, window.size = .6, M = 500){
  ##dat1 is first toolmark as 1 column matrix
  ##dat2 is second toolmark as 1 column matrix
  ...
  p1 <- ggplot() + 
    ...
    geom_segment(data = melt_corr_mat_smooth, aes(x = 1, y = max(row) - 2*M, xend = 2*M, yend = max(row)), size = 2) +
    geom_segment(data = melt_corr_mat_smooth, aes(x = max(col) - 2*M, y = 1, xend = max(col), yend = 2*M), size = 2) +
    geom_abline(intercept = max_diag_smooth, slope = 1, size = 2, lty = 2) + 
  ...
  p2 <- ggplot() + 
    ...
    geom_point(data = max_diag_locations_mid_point, aes(x = col, y = row), size = 2) +
    geom_path(data = search_area, aes(x = x, y = y), size = 2) +
    scale_fill_gradient(name = "Correlation", high = 'black', low = 'lightgray') + 
    theme(axis.title = element_text(size = 13),
          title = element_text(size = 13),
          plot.title = element_text(hjust=0.5))
